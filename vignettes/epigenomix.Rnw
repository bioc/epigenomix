...
head(fpkm[c(-2,-8), ])
@
The last six columns were not included in the Cuffdiff output, but were extracted from the annotation file given as input to Cuffdiff. Next, we construct an \Rclass{ExpressionSet} object so that we can handle RNA-seq data in the way as microarray data:
<<label=RNAseqToESet>>=
